...
public final class Interpolations {
	public static final Interpolation LINEAR = (query, delta, keyframes, start, end, scale, destination) -> {
		var vector3fc = keyframes[start].postTarget().evaluate(query);
		var vector3fc2 = keyframes[end].preTarget().evaluate(query);
	...
	public static final Interpolation CATMULL_ROM = (query, delta, keyframes, start, end, scale, destination) -> {
		Vector3fc p0 = keyframes[Math.max(0, start - 1)].postTarget().evaluate(query);
		Vector3fc p1 = keyframes[start].postTarget().evaluate(query);
		Vector3fc p2 = keyframes[end].postTarget().evaluate(query);
		Vector3fc p3 = keyframes[Math.min(keyframes.length - 1, end + 1)].postTarget().evaluate(query);
